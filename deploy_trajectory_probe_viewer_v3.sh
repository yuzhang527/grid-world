...
def fit_oof_cell_probes(
    ...
    for task_index, task in enumerate(task_names, 1):
        ...
        if len(classes) < 2:
            raise RuntimeError(f"Task {task} has fewer than two train classes: {classes}")
...
function render() {{
  ...
  document.getElementById('diag').textContent=wrong.length ? wrong.join('\n') : 'All decoded cells match the gold belief at this step.';
}}
...
echo "[deploy] syntax_check=PASS"
echo "[deploy] note=This v3 viewer uses validated 5-fold out-of-fold gold-belief predictions."
